...
@bp.post("/start-test")
def start_test():
    return jsonify(current_app.config["SYSTEM"]["composite_test_engine"].start_test())

@bp.post("/get-question")
def get_question():
    payload = request.get_json(force=True)
    sid = payload.get("session_id")
    if not sid:
        return jsonify({"error": "session_id zorunludur"}), 400
    try:
        return jsonify(current_app.config["SYSTEM"]["composite_test_engine"].get_question(sid))
    except KeyError:
...
def submit_answer():
    ...
    if not sid or not qid:
        return jsonify({"error": "session_id ve question_id zorunludur"}), 400
    try:
        return jsonify(current_app.config["SYSTEM"]["composite_test_engine"].submit_answer(sid, qid, answer))
    except KeyError:
        return jsonify({"error": "session bulunamadı"}), 404
...
def finish_test():
    ...
    if not sid:
        return jsonify({"error": "session_id zorunludur"}), 400
    try:
        return jsonify(current_app.config["SYSTEM"]["composite_test_engine"].finish_test(sid))
    except KeyError:
        return jsonify({"error": "session bulunamadı"}), 404
